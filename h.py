...
MODEL_CATEGORICAL_COLUMNS = ["estado_hogar", "parroquia_estudiante"]
...
def _cluster_summary(df: pd.DataFrame, institution_name: str) -> pd.DataFrame:
    ...
    grouped["cluster_id_original"] = grouped["cluster_id"].astype(int)
    cluster_labels = [f"Cluster {idx}" for idx in range(1, len(grouped) + 1)]
    cluster_map = dict(zip(grouped["cluster_id_original"], cluster_labels))
    grouped["cluster_id"] = grouped["cluster_id"].map(cluster_map)
...
def run_propensity_analysis(
    ...
    for university_name in universities:
        ...
        for institution_name in ["UDLA", university_name]:
            ...
            comparison.loc[mask, "cluster_id"] = labels
